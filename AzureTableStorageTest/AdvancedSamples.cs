...
namespace AzureTableStorageTest
{
    public class AdvancedSamples
    {
        ...
        private static async Task AdvancedDataOperationsAsync(CloudTable table)
        {
            ...
            await BatchInsertEntitiesAsync<Customer>(table);
            Console.WriteLine();
        ...
        private static async Task TableListingOperations(CloudTableClient tableClient)
        {
            try
            {
                ...
                await ListTablesWithPrefix(tableClient, "Customers");
        ...
        /// <returns>A Task object</returns>
        private static async Task<List<T>> BatchInsertEntitiesAsync<T>(CloudTable table)
        {
            try
            {
                ...
                for (int i = 0; i < 100; i++)
                {
                    batchOperation.InsertOrMerge(new Customer(i.ToString(), "Smith")
                    {
                ...
                List<T> results = (await table.ExecuteBatchAsync(batchOperation)).Select(c => (T)c.Result).ToList();
                //foreach (var res in results)
                //{
                //    var customerInserted = res.Result as Customer;
        ...
        private static async Task PartitionRangeQueryAsync(CloudTable table, string partitionKey, string startRowKey, string endRowKey)
        {
            try
            {
                ...
                do
                {
                    ...
                    TableQuerySegment<Customer> segment = await table.ExecuteQuerySegmentedAsync(rangeQuery, token);
                    ...
                    foreach (Customer entity in segment)
                    {
                        Console.WriteLine("\t Customer: {0},{1}\t{2}\t{3}", entity.PartitionKey, entity.RowKey, entity.Email, entity.PhoneNumber);
                    }
        ...
        private static async Task PartitionScanAsync(CloudTable table, string partitionKey)
        {
            try
            {
                TableQuery<Customer> partitionScanQuery =
            new TableQuery<Customer>().Where(TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, partitionKey));
                ...
                do
                {
                    TableQuerySegment<Customer> segment = await table.ExecuteQuerySegmentedAsync(partitionScanQuery, token);
                    token = segment.ContinuationToken;
                    foreach (Customer entity in segment)
                    {
                        Console.WriteLine("Customer: {0},{1}\t{2}\t{3}", entity.PartitionKey, entity.RowKey, entity.Email, entity.PhoneNumber);
